...
<!--      <p class="large-text">When the AI's confidence level is <span class="time-format">higher</span> than its accuracy of its prediction, we call this <span class="time-format">overconfident</span>. </p>-->
<!--      <p class="large-text">This means the AI often expresses high certainty about its outputs, but the outputs are not as reliable or correct as the AI's confidence suggests.</p>-->
<!--      <p class="large-text">For example, AI might claim to be 90% confident about its prediction, but in reality, it is correct only 70% of the time.</p>-->
<!--      <p class="large-text">When the AI's confidence level is <span class="time-format">lower</span> than its accuracy of its prediction, we call it <span class="time-format">underconfident</span>.</p>-->
<!--      <p class="large-text">It is the opposite of the overconfident one.</p>-->
<!--      <p class="large-text">For example, AI might claim to be only 70% confident about its prediction, but in reality, it is correct 90% of the time.</p>-->
<!--      <p class="large-text">When the AI's confidence level is <span class="time-format">essentially</span> consistent with its accuracy of its prediction, we call it <span class="time-format">appropriately confident</span>.</p>-->
<!--      <p class="large-text">For example, AI might claim to be 80% confident about its prediction, and it is also correct 80% of the time.</p>-->
<!--      <p class="large-text">-->
<!--        You will now complete the task with the assistance of an <span class="time-format">overconfident</span> AI.-->
<!--      </p>-->
      <p class="large-text">1. When the AI's confidence level is <span class="time-format">higher</span> than its accuracy of its prediction, we call this <span class="time-format">overconfident</span>. This means the AI often expresses high certainty about its outputs, but the outputs are not as reliable or correct as the AI's confidence suggests. For example, AI might claim to be 90% confident about its prediction, but in reality, it is correct only 70% of the time.</p>
      <p class="large-text">2. When the AI's confidence level is <span class="time-format">lower</span> than its accuracy of its prediction, we call it <span class="time-format">underconfident</span>. It is the opposite of the overconfident one. For example, AI might claim to be only 70% confident about its prediction, but in reality, it is correct 90% of the time.</p>
      <p class="large-text">3. When the AI's confidence level is <span class="time-format">essentially</span> consistent with its accuracy of its prediction, we call it <span class="time-format">appropriately confident</span>. For example, AI might claim to be 80% confident about its prediction, and it is also correct 80% of the time.</p>
      <p class="large-text important-text">
        You will now complete the task with the assistance of an <span class="time-format">underconfident</span> AI.
      </p>
      <div>
        <input style="margin-right: 10px; margin-top: 20px;" type="checkbox" id="confirmIntroCheckbox" v-model="isIntroConfirmed">
        <label for="confirmIntroCheckbox" class="large-text">Please confirm that you have read and understood the contents of this page.</label>
      </div>
      <el-button class="next-button" @click="startQuiz">
        Next {{ introButtonDisabled ? `(${introButtonCountdown})` : '' }}
      </el-button>
    </div>

    <div class="quiz-container" v-if="!showIntro">
...
<!--              <VueSlider v-model="confidenceValue" :min="0" :max="100" :tooltip="'always'" :height="10" style="width: 60%; margin-top: 20px;"></VueSlider>-->
          ...
          <!-- Countdown display -->
          <div class="countdown" v-if="timerActive && isEvenQuestion">
            <p>
              <span v-if="isCurrentAnswerCorrect" class="correct-answer">Congratulations, you got it right! Next question in: {{ countdown }} </span>
              <span v-else class="incorrect-answer">Unfortunately, you got it wrong. Next question in: {{ countdown }}</span>
            </p>
          </div>
    ...
    <div v-else class="completion-message">
      <p class="completion-text">Well done! You have completed all the questions. Below is a summary of your performance in the quiz:</p>
...
<!--      <p class="large-text">{{ completionMessage.message }}<span class="time-format">{{ elapsedTime }}</span>. {{ completionMessage.bonusMessage }}</p>-->
      ...
      <el-button style="margin-top: 30px;" @click="goToMenu">
        Back to Survey {{ buttonDisabled ? `(${buttonCountdown})` : '' }}
      </el-button>
    </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      buttonDisabled: true,
      introButtonDisabled: true,
      isConfirmed: false,
      isIntroConfirmed: false,
      showIntro: true,

      introButtonCountdown: 10, // Countdown time for the intro button
      introCountdownInterval: null, // To store the interval ID
      manualLabel: 'lower',
    }
  },
  ...
  created() {
    ...
    this.startIntroButtonCountdown();
  },
  ...
  methods: {
    ...
    startIntroButtonCountdown() {
      this.introCountdownInterval = setInterval(() => {
        if (this.introButtonCountdown > 0) {
          this.introButtonCountdown--;
        } else {
          this.introButtonDisabled = false;
          clearInterval(this.introCountdownInterval);
        }
      }, 1000);
    },
    ...
    startQuiz() {
      ...
      if (this.introButtonDisabled) {
        this.showMessage('warning', 'Please wait for the countdown to finish before proceeding.');
        return;
      }

      if (!this.isIntroConfirmed) {
        this.showMessage('warning', 'Please confirm that you have read and understood the contents of this page.');
        return;
      }

      // If everything is confirmed, proceed to the quiz
      this.showIntro = false; // Assuming you have a variable like this to control the display of the intro
    },
    ...
    beforeDestroy() {
      ...
      if (this.introCountdownInterval) {
        clearInterval(this.introCountdownInterval);
      }
    },
    ...
    goToMenu() {
      ...
      // Check if the confirmation checkbox is not checked
      if (!this.isConfirmed) {
        this.showMessage('warning', 'Please confirm that you have read and understood the contents of this page.');
        return;
      }
      ...
      const qualtricsUrl = `https://nus.syd1.qualtrics.com/jfe/form/SV_2lzxbCcSOysVio6/?PROLIFIC_PID=${PROLIFIC_PID}`;


      axios.post('https://ai-calibration-backend-da61b7d9f154.herokuapp.com/api/calibration_10_B', {
